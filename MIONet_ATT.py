...
lookback = 10  # Sliding window size
...
# Local attention branch network
...
class MultiHeadAttentionFusion(nn.Module):
    ...
    def forward(self, branch_pos_output, branch_vel_output):
        combined = torch.cat([branch_pos_output.unsqueeze(1), branch_vel_output.unsqueeze(1)], dim=1)  
        attn_output, _ = self.attention(combined, combined, combined) 
        return self.fc(attn_output.mean(dim=1))  


class MIONet-ATT(nn.Module):
